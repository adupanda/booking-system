...
export async function POST(request: Request) {
    ...
    try {
        ...
        // Included/free seats can be booked anywhere, including seats whose category is "paid".
        // Payment is needed only for seats beyond the remaining included allowance.
        const paidGuestSeatCount = Math.max(selectedSeats.length - includedRemaining, 0);
        const includedSeatCount = selectedSeats.length - paidGuestSeatCount;

        if (paidGuestSeatCount > selectedPaidSectionCount) {
            return NextResponse.json(
                {
                    success: false,
        ...
        const { data: paymentOrder, error: insertError } = await supabaseAdmin
            ...
            .insert({
                ...
                included_seat_count: includedSeatCount,
